...
export default function App() {
  ...
  if(!ready) return (
    <div style={{display:'flex',alignItems:'center',justifyContent:'center',
      height:'100vh',color:TXT2,fontSize:14,fontFamily:F}}>Loading…</div>
  );
            ...
            {EXP_CATS.map((c,i)=>{
              ...
              return (
                <Card key={c} style={{padding:'16px'}}>
                  <div style={{width:32,height:32,borderRadius:8,background:bgs[i],display:'flex',alignItems:'center',justifyContent:'center',marginBottom:10}}>
                    <i className="ti ti-cash" style={{fontSize:15,color:colors[i]}} aria-hidden/>
                  </div>
                  <div style={{fontSize:11,color:TXT2,fontWeight:600,marginBottom:4,textTransform:'uppercase',letterSpacing:'0.04em'}}>{c}</div>
            ...
            <div style={{overflowX:'auto'}}>
              <table style={{width:'100%',borderCollapse:'collapse'}}>
                <thead><tr style={{borderBottom:`2px solid ${TBORDER}`}}>
                  <th style={TH}>Month</th>{EXP_CATS.map(c=><th key={c} style={TH}>{c}</th>)}<th style={TH}>Total</th><th style={TH}/>
                </tr></thead>
                <tbody>
                  {fyMths.map(({label,key,year},ri)=>{
                    ...
                    return (
                      <tr key={key} style={{background:ri%2===0?CARD:'#FAFBFF',borderBottom:`1px solid ${TBORDER}`}}>
                        ...
                        <td style={TD}><button onClick={()=>{setExpF(r);setExpEdit(key);}} style={{background:'#EEEEFF',color:'#6366f1',border:'none',cursor:'pointer',padding:'4px 12px',borderRadius:8,fontSize:11,fontWeight:600,fontFamily:F}}>Edit</button></td>
                      </tr>
                    );
                  })}
